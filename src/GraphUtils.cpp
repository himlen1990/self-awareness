...
IplImage* drawFloatGraph(const float *arraySrc, int nArrayLength, IplImage *imageDst, float minV, float maxV, int width, int height, const char* lineLabel, char* graphLabel, bool showScale)
{
	int w = width;
	int h = height;
...
// Write the graph label, if desired
	if (lineLabel != NULL && strlen(lineLabel) > 0) {
		//cvInitFont(&font,CV_FONT_HERSHEY_PLAIN, 0.5,0.7, 0,1,CV_AA);
		cvPutText(imageGraph, lineLabel, cvPoint(30, 10*countGraph), &font,colorGraph );	// black text
	}
